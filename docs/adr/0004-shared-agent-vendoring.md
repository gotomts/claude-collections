...
**Superseded by [ADR-0010](0010-shared-as-plugin-agent-namespace.md)** (2026-07-28)。vendoring 方式は同名 agent の registry 衝突を招くことが判明したため廃止し、`shared/` を plugin 化して単一実体で配る方式に移行した。本 ADR のうち「`shared/agents/` は collection 非依存の中立語彙で書く」原則のみ ADR-0010 が継承する。
