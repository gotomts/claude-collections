...
- **出力レイアウト**：repo の `docs/discovery/` に集約。`anchors/`（G1 対話から AI が書く）・`planning/`（企画層・番号固定 ＋ `feature-details.md`）・`design/`（design-concept/system/tokens・`screens.md`・`screen-specs/<area>/`）・`brief.md`・`DECISIONS.md`。
