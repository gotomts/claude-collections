...
- **In**: spec-first-superpowers コレクションの新設、shared/skills/finish-stage-pr の後方互換改修、root の marketplace.json 登録、CONTEXT-MAP.md 索引追加、**セキュリティレビュー** (Spec フェーズの設計レベル + 実装後 STOP POINT 2 のコードレベル)、**監査ログ** (agent dispatch log を 5 成果物に追記)、**コンプライアンス** (機微情報チェック / ライセンスチェック / AI 利用ポリシー案内、いずれも trigger / 案内のみで強制はしない)
