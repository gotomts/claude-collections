---
title: enhance-superpowers コレクション新設 — 設計
date: 2026-06-25
branch: feat/enterprise-superpowers-customization
status: draft (brainstorming 段階、user 承認待ち)
---

# enhance-superpowers コレクション新設 — 設計

## 概要

`claude-collections` リポジトリに、superpowers (公式) を base に拡張したコレクション **`enhance-superpowers`** を新設する。`~/.claude/CLAUDE.local.md` に蓄積されてきた「コミット前提に直したローカル開発フローカスタマイズ」(5 成果物 / Spec 先行 / GWT テスト運用 / code-review 採用Skip / 指摘対応規律 / 設計思想 / コードコメント方針) を skill+agent コレクションとしてパッケージ化し、PC 以外の環境 (他 PC / 業務 repo) でも install して同じフローで開発できるようにする。
...
- **監査ログ (agent dispatch log)**: 各 skill で agent dispatch した際、`いつ / 誰を / 何のために dispatch したか + 回答要約` を該当 5 成果物 (design / plan / summary / gwt / review-response) に追記。「なぜこの設計を採ったか」のトレースに agent の助言も含めて説明可能にする
- **機微情報チェック (法令 / 業界規制 trigger)**: enhance-brainstorming Phase 3 の security-engineer dispatch 時に「個人情報 / 決済データ / 医療データ / 認証情報を扱う設計か?」をチェックリスト形式で確認、該当したら適用規制 (GDPR / 個人情報保護法 / PCI-DSS / HIPAA 等) を提示して user に確認させる。具体的な規制チェックは外部ツール / 法務に委ねる (本コレクションは trigger 提示のみ)
- **ライセンスチェック (OSS ライセンス遵守)**: enhance-brainstorming Phase 4 (plan 生成後) に「plan で追加予定の依存ライブラリ一覧」を抽出、各ライブラリのライセンスを確認 (license-checker 等の推奨案内含む)、制限ライセンス (GPL / AGPL / SSPL / 商用制限あり) が含まれる場合に user に警告
- **AI 利用ポリシー案内**: 各 skill の Step 1 で `.ai-restrictions.md` (または同等のプロジェクト固有ファイル) を Read、存在すれば内容を表示して制約を user に案内 (ファイル無ければスキップ)。強制ではなく案内のみ、ファイル形式は環境依存
...
- **In**: enhance-superpowers コレクションの新設、shared/skills/finish-stage-pr の後方互換改修、root の marketplace.json 登録、CONTEXT-MAP.md 索引追加、**セキュリティレビュー** (Spec フェーズの設計レベル + 実装後 STOP POINT 2 のコードレベル)、**監査ログ** (agent dispatch log を 5 成果物に追記)、**コンプライアンス** (機微情報チェック / ライセンスチェック / AI 利用ポリシー案内、いずれも trigger / 案内のみで強制はしない)
- **Out**: エンプラ特有要素のうち **変更管理 / 承認フロー** は本コレクションには含めない。必要なら別コレクション / 別 ADR で後続検討
- **Out**: コード変更ログ (git log でカバー) / 本番デプロイ履歴 (CI/CD 領域) は既存ツールで十分なため本コレクションのスコープ外
...
└── .claude-plugin/
    └── marketplace.json                        # enhance-superpowers entry を追加
```
...
- `shared/agents/` → `make sync COLLECTION=enhance-superpowers` で `agents/` に generated file として展開
...
- agent vendoring (`shared/`) を再利用して職種境界を保つ (enhance-superpowers 固有 agent は持たない、初期)

### indie-studio との棲み分け

| 観点 | indie-studio | enhance-superpowers |
|---|---|---|
...
- **invoke trigger**: 機能開発開始時にユーザーが `enhance-superpowers:enhance-brainstorming` を呼ぶ
- **動作**:
  1. ブランチ確認 → `docs/superpowers/{branch}/` 準備 (commit 前提、worktree 退避なし) → **`.ai-restrictions.md` (または同等のプロジェクト固有 AI 利用ポリシー) を Read して制約があれば user に案内 (H)**
  2. Phase 1: 会話で問題理解 → 2-3 アプローチ提示 → **`software-architect` を dispatch してアプローチ案の Clean Architecture + SOLID 整合性レビュー** → **dispatch log を Phase 2 で生成する summary.md の「レビュー履歴」セクションへの追記対象として保持 (B)**
  3. Phase 2: templates/summary.md から summary.md を生成 → **`software-architect` を dispatch して「方式の要点」「効いている設計判断」を SOLID/YAGNI 観点でレビュー** → **summary.md の末尾に「## レビュー履歴」セクションを追加し、Phase 1 + Phase 2 の dispatch log (時刻 / agent / 目的 / 回答要約) を追記 (B)** → user 承認 + commit (認識齟齬 検出ポイント①)
  4. Phase 3: 合意済み summary を context として `superpowers:brainstorming` に委譲 → design.md → **`software-architect` を dispatch して design 全体の SOLID / モジュール境界レビュー** → **続けて `security-engineer` を常時能動 dispatch して design のセキュリティレビュー (認証 / 認可 / データ取扱 / 外部入力 / シークレット / 通信 / コード実行 等の観点) + 機微情報チェックリスト (個人情報 / 決済データ / 医療データ / 認証情報を扱う設計か?) を確認、該当したら適用規制 (GDPR / 個人情報保護法 / PCI-DSS / HIPAA 等) を提示して user に確認 (E)** → **design.md の末尾に「## レビュー履歴」セクションを追加し dispatch log を追記 (B)** → user 承認 + commit
  5. Phase 4: `superpowers:writing-plans` invoke → plan.md → **`qa-engineer` を dispatch して plan のテスト戦略の段取り妥当性レビュー** → **続けて `security-engineer` を常時能動 dispatch して plan のセキュリティ観点 (セキュリティテスト / 脅威モデリングの段取り / 機微データ取扱の手順) レビュー** → **続けて plan で追加予定の依存ライブラリを抽出 → 各ライブラリのライセンスを確認 (license-checker 等の推奨案内、user 確認 1 問)、制限ライセンス (GPL / AGPL / SSPL / 商用制限あり) は警告 (G)** → **plan.md の末尾に「## レビュー履歴」セクションを追加し dispatch log を追記 (B)** → user 承認 + commit
  6. Phase 5: templates/gwt.md から gwt.md を生成 → **`qa-engineer` を dispatch して AC の網羅性 (異常系 / 境界値 / 空状態) レビュー** → **gwt.md の末尾に「## レビュー履歴」セクションを追加し dispatch log を追記 (B)** → user 承認 + commit (認識齟齬 検出ポイント②)
  7. Phase 6: templates/pr-description.md から pr-description.md を生成 → user 承認 + commit (認識齟齬 検出ポイント③)。**pr-description は frontmatter なし 3 セクション固定 = 最小構造なのでレビュー履歴はここに追加せず、summary / design / plan / gwt 側に集約 (B 例外)**
  8. **Phase 1 / 2 / 5 / 6 でもセキュリティ箇所が検出されたら**: `security-engineer` を任意 dispatch (Phase 3/4 の常時 dispatch とは別)、dispatch log は該当 Phase の成果物 (summary / gwt) のレビュー履歴に追記 (B)
...
- **規律明示 (SKILL.md に記載)**:
  ...
  - **agent dispatch log は該当 5 成果物の「レビュー履歴」セクションに必ず追記 (B)、形式は ADR-0007 で定める**
  - **Phase 3 で機微情報チェックリスト + 適用規制 trigger を提示 (E)**
  - **Phase 4 で依存ライブラリのライセンスチェックを実施 (G)**
  - **各 skill の Step 1 で `.ai-restrictions.md` を Read して AI 利用ポリシーを案内 (H)、ファイル無ければスキップ**

#### gwt-test 詳細

- **invoke trigger**: 実装完了後、ユーザーが `gwt-test` を呼ぶ or enhance-brainstorming からの再開連鎖
- **動作**:
  1. プロジェクトルートの README を Read (テストアカウント / 起動コマンド把握) + **`.ai-restrictions.md` を Read して AI 利用ポリシーを user に案内 (H)**
  ...
  5. AC 未達 → **`qa-engineer` を dispatch して差し戻し findings の言語化 + テストコード同期確認** → `gwt.md` 変更履歴に逆時系列追記 → **gwt.md のレビュー履歴セクションに dispatch log 追記 (B)** → 実装差し戻し提案
  6. dev server / docker を必ず停止 (`lsof` / `docker ps` で残存確認)
  7. STOP POINT 2 (code-review skill 手動実行) を user に明示する際、**「`code-review` (CodeRabbit) の実行に加えて `security-engineer` を能動 dispatch して security-focused なコードレビューを 1 回実施する」** ことを案内に必ず含める (実装後の機械的レビュー = CodeRabbit、人格を持つ専門家レビュー = security-engineer、の両輪で実装フェーズのセキュリティを担保)。**security-engineer dispatch 結果は write-review-response 内で review-response.md のレビュー履歴セクションに追記 (B)**
- **規律明示**:
  ...
  - dispatch log は gwt.md / review-response.md のレビュー履歴セクションに追記 (B)
  - Step 1 で AI 利用ポリシー (.ai-restrictions.md) を Read して案内 (H)

#### write-review-response 詳細

- **invoke trigger**: code-review skill 実行後、ユーザーが `write-review-response` を呼ぶ or gwt-test からの再開連鎖
- **動作**:
  1. templates/review-response.md を読み込み + **`.ai-restrictions.md` を Read して AI 利用ポリシーを user に案内 (H)**
  ...
  4. **上書き運用** で `{date}-{slug}-review-response.md` を保存 (最新ラウンドのみ保持) → **review-response.md の末尾「## レビュー履歴」セクションに dispatch log を追記 (B)、gwt-test の STOP POINT 2 で実施した security-engineer のコードセキュリティレビュー結果もここに集約**
  5. 採用分を実装に反映 (← user 作業 or AI 作業) → **再 push 前に `code-reviewer` を dispatch して修正コードの差し戻しレビュー** → **dispatch log を review-response.md レビュー履歴に追記 (B)**
- **規律明示**:
  ...
  - dispatch log を review-response.md のレビュー履歴セクションに追記 (B、gwt-test の security-engineer コードレビュー結果もここに集約)
  - Step 1 で AI 利用ポリシー (.ai-restrictions.md) を Read して案内 (H)
...
- **動作**:
  1. `docs/superpowers/{branch}/*-pr-description.md` を Read + **`.ai-restrictions.md` を Read して AI 利用ポリシーを user に案内 (H)**
...
- **規律明示**:
  - Step 1 で AI 利用ポリシー (.ai-restrictions.md) を Read して案内 (H)
  - finish-spec-pr 自体は agent dispatch しない (mechanical な操作のみ)、dispatch log 追記もなし

### 取り込み agent (4 体)

`enhance-superpowers/.claude-plugin/dependencies.json`:
...
| skill / ステップ | 専門家 agent | dispatch 種別 | 目的 | dispatch log 追記先 (B) |
|---|---|---|---|---|
| enhance-brainstorming Phase 1 (アプローチ提示) | `software-architect` | 能動 | アプローチ案の Clean Architecture + SOLID 整合性レビュー | summary.md (Phase 2 で追記) |
| enhance-brainstorming Phase 2 (summary 生成後) | `software-architect` | 能動 | 「方式の要点」「効いている設計判断」を SOLID/YAGNI 観点でレビュー | summary.md |
| enhance-brainstorming Phase 3 (design 生成後) | `software-architect` + **`security-engineer`** | 能動 (両方) | software-architect: SOLID / モジュール境界 / **security-engineer: design セキュリティレビュー + 機微情報チェックリスト (E) → 適用規制 trigger 提示** | design.md |
| enhance-brainstorming Phase 4 (plan 生成後) | `qa-engineer` + **`security-engineer`** | 能動 (両方) | qa-engineer: テスト戦略 / **security-engineer: plan セキュリティ観点 + 依存ライブラリのライセンスチェック (G) → 制限ライセンス warning** | plan.md |
| enhance-brainstorming Phase 5 (gwt 生成後) | `qa-engineer` | 能動 | AC の網羅性 (異常系 / 境界値 / 空状態) レビュー | gwt.md |
| enhance-brainstorming Phase 6 (pr-description 生成後) | (なし) | — | 動作確認方法は user が責任を持つ。**pr-description は最小構造維持のためレビュー履歴を追記しない (B 例外)** | (追記なし) |
| enhance-brainstorming Phase 1 / 2 / 5 / 6 (セキュリティ箇所検出時) | `security-engineer` | 任意 | Phase 3/4 の常時 dispatch とは別、検出時のみ追加 dispatch | 該当 Phase の成果物 |
| **STOP POINT 1 (実装フェーズ)** | (user / 実装 AI が任意で dispatch) | 案内 | **推奨 agent 利用パターンを案内** (`software-architect` / `code-reviewer` / `security-engineer`) | 実装側に委ねる (本コレクション管理外) |
| gwt-test (AC 達成時) | (なし) | — | チェックリスト更新のみ | (なし) |
| gwt-test (AC 未達発覚時) | `qa-engineer` | 能動 | 差し戻し findings の言語化、テストコード同期確認 | gwt.md |
| **STOP POINT 2 (code-review skill 手動実行)** | **`security-engineer`** + (CodeRabbit) | **案内 + 能動 dispatch** | code-review (CodeRabbit) 実行 + security-engineer の security-focused コードレビューの両輪 | review-response.md (write-review-response で集約) |
| write-review-response (判定迷い時) | `code-reviewer` | 能動 | 採用/Skip 判定の補助 (特に false positive 疑い時) | review-response.md |
| write-review-response (セキュリティ系指摘) | `security-engineer` | 能動 | 採用判定にセキュリティ観点を追加 | review-response.md |
| write-review-response (採用後の修正、再 push 前) | `code-reviewer` | 能動 | 修正コードの差し戻しレビュー | review-response.md |
| finish-spec-pr (PR 作成全工程) | (なし) | — | mechanical な操作のみ | (なし) |
| 全 skill Step 1 (H) | (agent dispatch なし、ファイル Read のみ) | — | `.ai-restrictions.md` を Read して AI 利用ポリシーを案内 (ファイル無ければスキップ) | (該当成果物の Step 1 ログには記録するが、専門家 dispatch ではないため B の対象外) |

統一原則: **「agent を取り込んだら、必ず使う場面を skill ステップに織り込む」+ 「dispatch したら必ず該当成果物のレビュー履歴セクションに追記する」(B)**。任意 dispatch 表現は (a) STOP POINT のような skill 制御外、または (b) 検出条件付きの追加 dispatch (Phase 3/4 以外の Phase でのセキュリティ箇所検出時等) でのみ許容。

## データフロー

```
ユーザーが意識的に呼ぶのは enhance-superpowers:enhance-brainstorming 1 つ
...
`enhance-superpowers/docs/adr/`:

| ADR | タイトル |
|---|---|
| 0001 | collection-scope-and-naming (= brainstorming 拡張 + 5 成果物 + 後工程連鎖、命名: enhance-superpowers) |
...
| 0007 | audit-trail-dispatch-log (agent dispatch log を 5 成果物のレビュー履歴セクションに追記する形式と運用) |
| 0008 | sensitive-data-check-in-spec-phase (Phase 3 で機微情報チェックリスト + 適用規制 trigger 提示の手順) |
| 0009 | license-check-in-plan-phase (Phase 4 で依存ライブラリのライセンスチェックと制限ライセンス warning の手順) |
| 0010 | ai-utilization-policy-loading (各 skill Step 1 で .ai-restrictions.md を Read して案内する仕様、ファイル形式と未存在時挙動) |
...
- enhance-superpowers の `finish-spec-pr` は `body-source-path={pr-description.md path}` を指定して呼ぶので、新分岐 (file 内容 = body) が動作
...
| 変更先 | 内容 | 影響度 |
|---|---|---|
| `enhance-superpowers/` 配下 (新規) | コレクション一式 (CONTEXT.md / docs/adr / skills / templates / .claude-plugin) | 新規追加のみ |
| `enhance-superpowers/agents/` | `make sync` で生成 (手編集禁止) | sync コマンドで生成 |
...
| root `.claude-plugin/marketplace.json` | enhance-superpowers entry を追加 | 配布対象に追加 |
| root `CONTEXT-MAP.md` | enhance-superpowers を索引に追加 | 索引追加のみ |
...
- enhance-superpowers/skills/ 内のディレクトリ構造 (SKILL.md 直下 / sub-files 配下)
...
- [ ] `enhance-superpowers/` ディレクトリ作成 + CONTEXT.md / .claude-plugin / templates / 4 skill 配置
- [ ] `dependencies.json` 宣言 → `make sync COLLECTION=enhance-superpowers` で agents/ + skills/finish-stage-pr 取り込み完了
