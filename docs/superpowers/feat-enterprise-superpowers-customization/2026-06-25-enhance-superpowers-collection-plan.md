# enhance-superpowers コレクション新設 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `~/.claude/CLAUDE.local.md` で運用してきたローカル開発フローカスタマイズを `enhance-superpowers` コレクションとして `claude-collections` repo に新設し、他環境にも持ち出せる skill+agent パッケージにする。
...
- **ファイル配置**: spec / plan / summary / gwt / pr-description / ADR は `docs/superpowers/{branch}/` (worktree 同居、commit 前提)、コレクション本体は `enhance-superpowers/` 配下
- **shared/ の取扱**: `shared/agents/` `shared/skills/` の真実源は touch せず、`make sync COLLECTION=enhance-superpowers` で `enhance-superpowers/agents/` `enhance-superpowers/skills/finish-stage-pr/` に generated file として展開、generated file は手編集禁止
...
.claude-plugin/marketplace.json                        # enhance-superpowers entry 追加
...
- Produces: 改修後の `finish-stage-pr` skill (`body-source-path` 任意引数を受け取れる)。indie-studio は引数なしで既存挙動、enhance-superpowers の `finish-spec-pr` は `body-source-path={pr-description.md path}` を指定して呼ぶ
...
enhance-superpowers の finish-spec-pr が pr-description.md を body として渡す
用途で利用予定 (ADR-0004)。
...
## Task 2: `enhance-superpowers/` 骨格作成 (CONTEXT.md + plugin.json + ディレクトリ)

**Files:**
- Create: `enhance-superpowers/CONTEXT.md`
- Create: `enhance-superpowers/.claude-plugin/plugin.json`
- Create: `enhance-superpowers/docs/adr/.gitkeep` (ADR 0001-0010 は Task 5/6 で配置)
- Create: `enhance-superpowers/skills/.gitkeep` (実体は Task 7-10 で配置)
- Create: `enhance-superpowers/agents/.gitkeep` (実体は Task 4 で make sync 生成)
- Create: `enhance-superpowers/templates/.gitkeep` (実体は Task 3 で配置)
...
Run:
```bash
mkdir -p enhance-superpowers/{docs/adr,skills,agents,templates,.claude-plugin}
touch enhance-superpowers/docs/adr/.gitkeep
touch enhance-superpowers/skills/.gitkeep
touch enhance-superpowers/agents/.gitkeep
touch enhance-superpowers/templates/.gitkeep
ls -la enhance-superpowers/
```
Expected: 5 ディレクトリ (docs/adr, skills, agents, templates, .claude-plugin) と .gitkeep 4 個が見える。

- [ ] **Step 2: CONTEXT.md を作成**

Write `enhance-superpowers/CONTEXT.md`:
```markdown
...
Write `enhance-superpowers/.claude-plugin/plugin.json`:
```json
{
  "$schema": "https://json.schemastore.org/claude-code-plugin-manifest.json",
  "name": "enhance-superpowers",
...
```bash
ls -la enhance-superpowers/
cat enhance-superpowers/CONTEXT.md | head -20
cat enhance-superpowers/.claude-plugin/plugin.json
```
Expected: ディレクトリ 5 個 + CONTEXT.md + .claude-plugin/plugin.json が存在、CONTEXT.md 冒頭は「enhance-superpowers コレクション」、plugin.json は enhance-superpowers として valid JSON。

- [ ] **Step 5: Commit**

```bash
git add enhance-superpowers/CONTEXT.md \
        enhance-superpowers/.claude-plugin/plugin.json \
        enhance-superpowers/docs/adr/.gitkeep \
        enhance-superpowers/skills/.gitkeep \
        enhance-superpowers/agents/.gitkeep \
        enhance-superpowers/templates/.gitkeep
git commit -m "$(cat <<'EOF'
feat(enhance-superpowers): コレクション骨格 (CONTEXT.md + plugin.json + ディレクトリ構造)

claude-collections repo に enhance-superpowers コレクションを新設する第一歩。
...
## Task 3: templates 4 種を `enhance-superpowers/templates/` に配置

**Files:**
- Create: `enhance-superpowers/templates/summary.md`
- Create: `enhance-superpowers/templates/gwt.md`
- Create: `enhance-superpowers/templates/pr-description.md`
- Create: `enhance-superpowers/templates/review-response.md`
- Delete: `enhance-superpowers/templates/.gitkeep`
...
Write `enhance-superpowers/templates/summary.md`:
```markdown
---
...
Write `enhance-superpowers/templates/gwt.md`:
```markdown
---
...
Write `enhance-superpowers/templates/pr-description.md`:
```markdown
## やったこと
...
Write `enhance-superpowers/templates/review-response.md`:
```markdown
---
...
Run:
```bash
rm enhance-superpowers/templates/.gitkeep
ls -la enhance-superpowers/templates/
```
...
```bash
git add enhance-superpowers/templates/
git commit -m "$(cat <<'EOF'
feat(enhance-superpowers): templates 4 種を配置 (summary / gwt / pr-description / review-response)

5 成果物のうち summary / gwt / pr-description と、レビュー対応の review-response の
4 種 template を enhance-superpowers/templates/ に同梱。~/.claude/local-templates/
...
**Files:**
- Create: `enhance-superpowers/.claude-plugin/dependencies.json`
- Auto-generated: `enhance-superpowers/agents/{code-reviewer,qa-engineer,software-architect,security-engineer}.md` (make sync 生成、手編集禁止)
- Auto-generated: `enhance-superpowers/skills/finish-stage-pr/SKILL.md` (make sync 生成、手編集禁止)
- Delete: `enhance-superpowers/agents/.gitkeep` (sync で実体ができたら不要)
...
Write `enhance-superpowers/.claude-plugin/dependencies.json`:
```json
{
  "shared": {
    "agents": [
...
Run: `make sync COLLECTION=enhance-superpowers`
Expected: `Synced: enhance-superpowers/agents/{code-reviewer,qa-engineer,software-architect,security-engineer}.md` および `Synced: enhance-superpowers/skills/finish-stage-pr/SKILL.md` の log。エラーなく完了。

- [ ] **Step 3: 取り込み結果を確認**

Run:
```bash
ls -la enhance-superpowers/agents/
ls -la enhance-superpowers/skills/finish-stage-pr/
head -10 enhance-superpowers/agents/code-reviewer.md
```
...
Run: `rm enhance-superpowers/agents/.gitkeep`
...
```bash
git add enhance-superpowers/.claude-plugin/dependencies.json \
        enhance-superpowers/agents/ \
        enhance-superpowers/skills/finish-stage-pr/
git commit -m "$(cat <<'EOF'
feat(enhance-superpowers): dependencies.json + make sync で agents 4 + finish-stage-pr 取り込み
...
**Files:**
- Create: `enhance-superpowers/docs/adr/0001-collection-scope-and-naming.md`
- Create: `enhance-superpowers/docs/adr/0002-five-deliverables-and-order.md`
- Create: `enhance-superpowers/docs/adr/0003-skill-chain-and-stop-points.md`
- Create: `enhance-superpowers/docs/adr/0004-shared-skills-finish-stage-pr-extension.md`
- Create: `enhance-superpowers/docs/adr/0005-agent-vendoring.md`
- Create: `enhance-superpowers/docs/adr/0006-superpowers-brainstorming-context-delegation.md`

**Interfaces:**
- Consumes: 設計 doc / summary.md (`docs/superpowers/feat-enterprise-superpowers-customization/2026-06-25-enhance-superpowers-collection-{design,summary}.md`) の内容
...
Write `enhance-superpowers/docs/adr/0001-collection-scope-and-naming.md`:
```markdown
# 0001. コレクションのスコープと命名
...
命名は **`enhance-superpowers`** — 「Spec フェーズで 5 成果物 (特に pr-description) を先行確定」がコレクションの最大の独自性を直撃。enterprise や production よりも特徴を捉える。
...
Write `enhance-superpowers/docs/adr/0002-five-deliverables-and-order.md`:
```markdown
...
Write `enhance-superpowers/docs/adr/0003-skill-chain-and-stop-points.md`:
```markdown
...
Write `enhance-superpowers/docs/adr/0004-shared-skills-finish-stage-pr-extension.md`:
```markdown
...
Write `enhance-superpowers/docs/adr/0005-agent-vendoring.md`:
```markdown
...
Write `enhance-superpowers/docs/adr/0006-superpowers-brainstorming-context-delegation.md`:
```markdown
...
Run: `ls -la enhance-superpowers/docs/adr/`
Expected: 0001-0006 の 6 ファイル + .gitkeep (まだ残っているなら次 step で削除)

- [ ] **Step 8: .gitkeep 削除 + Commit**

```bash
rm enhance-superpowers/docs/adr/.gitkeep 2>/dev/null || true
git add enhance-superpowers/docs/adr/0001-collection-scope-and-naming.md \
        enhance-superpowers/docs/adr/0002-five-deliverables-and-order.md \
        enhance-superpowers/docs/adr/0003-skill-chain-and-stop-points.md \
        enhance-superpowers/docs/adr/0004-shared-skills-finish-stage-pr-extension.md \
        enhance-superpowers/docs/adr/0005-agent-vendoring.md \
        enhance-superpowers/docs/adr/0006-superpowers-brainstorming-context-delegation.md
[ -f enhance-superpowers/docs/adr/.gitkeep ] || git rm --cached enhance-superpowers/docs/adr/.gitkeep 2>/dev/null || true
git commit -m "$(cat <<'EOF'
docs(enhance-superpowers): ADR 0001-0006 を起草 (設計判断系)
...
**Files:**
- Create: `enhance-superpowers/docs/adr/0007-audit-trail-dispatch-log.md`
- Create: `enhance-superpowers/docs/adr/0008-sensitive-data-check-in-spec-phase.md`
- Create: `enhance-superpowers/docs/adr/0009-license-check-in-plan-phase.md`
- Create: `enhance-superpowers/docs/adr/0010-ai-utilization-policy-loading.md`
...
Write `enhance-superpowers/docs/adr/0007-audit-trail-dispatch-log.md`:
```markdown
...
Write `enhance-superpowers/docs/adr/0008-sensitive-data-check-in-spec-phase.md`:
```markdown
...
Write `enhance-superpowers/docs/adr/0009-license-check-in-plan-phase.md`:
```markdown
...
Write `enhance-superpowers/docs/adr/0010-ai-utilization-policy-loading.md`:
```markdown
...
```bash
git add enhance-superpowers/docs/adr/0007-audit-trail-dispatch-log.md \
        enhance-superpowers/docs/adr/0008-sensitive-data-check-in-spec-phase.md \
        enhance-superpowers/docs/adr/0009-license-check-in-plan-phase.md \
        enhance-superpowers/docs/adr/0010-ai-utilization-policy-loading.md
git commit -m "$(cat <<'EOF'
docs(enhance-superpowers): ADR 0007-0010 を起草 (追加機能系)
...
**Files:**
- Create: `enhance-superpowers/skills/enhance-brainstorming/SKILL.md`
...
Write `enhance-superpowers/skills/enhance-brainstorming/SKILL.md`:
```markdown
---
...
Run: `Read enhance-superpowers/skills/enhance-brainstorming/SKILL.md`
Expected: frontmatter (name / description / argument-hint / allowed-tools / maintainer) + 動作 9 ステップ + 規律明示 + 失敗時の挙動 + 関連 ADR が全て揃っている。

- [ ] **Step 3: Commit**

```bash
git add enhance-superpowers/skills/enhance-brainstorming/SKILL.md
git commit -m "$(cat <<'EOF'
feat(enhance-superpowers): enhance-brainstorming skill を実装
...
**Files:**
- Create: `enhance-superpowers/skills/gwt-test/SKILL.md`
...
Write `enhance-superpowers/skills/gwt-test/SKILL.md`:
```markdown
---
name: gwt-test
description: |
...
Run: `Read enhance-superpowers/skills/gwt-test/SKILL.md`
Expected: frontmatter + 動作 7 ステップ + 規律明示 + 失敗時の挙動 + 関連 ADR が揃っている。

- [ ] **Step 3: Commit**

```bash
git add enhance-superpowers/skills/gwt-test/SKILL.md
git commit -m "$(cat <<'EOF'
feat(enhance-superpowers): gwt-test skill を実装
...
**Files:**
- Create: `enhance-superpowers/skills/write-review-response/SKILL.md`
...
Write `enhance-superpowers/skills/write-review-response/SKILL.md`:
```markdown
---
...
Run: `Read enhance-superpowers/skills/write-review-response/SKILL.md`
Expected: frontmatter + 動作 5 ステップ + 規律明示 + 失敗時の挙動 + 関連 ADR が揃っている。

- [ ] **Step 3: Commit**

```bash
git add enhance-superpowers/skills/write-review-response/SKILL.md
git commit -m "$(cat <<'EOF'
feat(enhance-superpowers): write-review-response skill を実装
...
**Files:**
- Create: `enhance-superpowers/skills/finish-spec-pr/SKILL.md`
...
Write `enhance-superpowers/skills/finish-spec-pr/SKILL.md`:
```markdown
---
name: finish-spec-pr
description: |
...
Run: `Read enhance-superpowers/skills/finish-spec-pr/SKILL.md`
Expected: frontmatter + 動作 6 ステップ + 規律明示 + 失敗時の挙動 + 関連 ADR が揃っている。

- [ ] **Step 3: Commit**

```bash
git add enhance-superpowers/skills/finish-spec-pr/SKILL.md
git commit -m "$(cat <<'EOF'
feat(enhance-superpowers): finish-spec-pr skill を実装
...
**Files:**
- Modify: `.claude-plugin/marketplace.json` (plugins 配列に enhance-superpowers entry 追加)
- Modify: `CONTEXT-MAP.md` (コレクションテーブルに行追加)

**Interfaces:**
- Consumes: Task 2 で作成した `enhance-superpowers/.claude-plugin/plugin.json` の name / description (整合)
- Produces: marketplace 経由で enhance-superpowers が他人にも install 可能になる、CONTEXT-MAP から enhance-superpowers が索引で辿れる
...
- [ ] **Step 2: enhance-superpowers entry を追加する Edit**
...
cat CONTEXT-MAP.md
```
Expected: marketplace.json に enhance-superpowers entry、CONTEXT-MAP に索引行 + shared/ 章更新。
...
git commit -m "$(cat <<'EOF'
feat: enhance-superpowers を marketplace 登録 + CONTEXT-MAP 索引追加

- .claude-plugin/marketplace.json: enhance-superpowers entry を追加
  (category: development-workflow、indie-studio と並列)
- CONTEXT-MAP.md: コレクションテーブルに enhance-superpowers 行追加、
  shared/ 章を skills 含む形に update (start-stage-branch / finish-stage-pr の
  vendoring と enhance-superpowers の取り込み 4 体を明示)
...
Run:
```bash
# Claude Code 上で /enhance-superpowers:enhance-brainstorming を呼んだときに
# SKILL.md が正しく読み込まれるか確認。本検証は Claude Code セッション内で実行する
# 必要があり、本プランの範囲を超えるため、user が手動で確認することを想定。
ls -la enhance-superpowers/skills/enhance-brainstorming/SKILL.md
echo "user manual verification: Claude Code で /enhance-superpowers:enhance-brainstorming を呼んで Y 方式が動作するか確認してください"
```
...
Run:
```bash
grep -n "を能動 dispatch" enhance-superpowers/skills/enhance-brainstorming/SKILL.md
grep -n "を能動 dispatch" enhance-superpowers/skills/gwt-test/SKILL.md
grep -n "を能動 dispatch" enhance-superpowers/skills/write-review-response/SKILL.md
```
Expected:
...
# Edit enhance-superpowers/skills/enhance-brainstorming/SKILL.md
# git add enhance-superpowers/skills/enhance-brainstorming/SKILL.md
# git commit -m "fix(enhance-superpowers): enhance-brainstorming の誤字訂正"
...
**Files:**
- Modify: `docs/superpowers/feat-enterprise-superpowers-customization/2026-06-25-enhance-superpowers-collection-pr-description.md` (Spec フェーズで未作成、本タスクで新規作成 + 仕上げ。**ただし enhance-brainstorming の Phase 6 を未実行のため、本タスクで作成する**)
...
Write `docs/superpowers/feat-enterprise-superpowers-customization/2026-06-25-enhance-superpowers-collection-pr-description.md`:
```markdown
## やったこと

- `claude-collections` repo に `enhance-superpowers` コレクションを新設 (indie-studio と並列の plugin collection)
...
- templates 4 種 (summary / gwt / pr-description / review-response) を enhance-superpowers/templates/ に同梱
...
- 本 PR は enhance-superpowers コレクションの初リリース。indie-studio のような 5 ステージ × 5 ゲート × 多職種 director モデルとは思想が異なる (superpowers の直線フローを尊重)、用途で棲み分ける
...
2. `make sync COLLECTION=enhance-superpowers` を実行して agents/skills が正しく取り込まれることを確認
3. `make verify` で drift なし (exit code 0) を確認
4. Claude Code 上で `/enhance-superpowers:enhance-brainstorming <topic>` を呼んで以下を確認:
...
```bash
git add docs/superpowers/feat-enterprise-superpowers-customization/2026-06-25-enhance-superpowers-collection-pr-description.md
git commit -m "$(cat <<'EOF'
docs: enhance-superpowers の pr-description.md を作成
...
Run (Claude Code 上で):
```
/enhance-superpowers:finish-spec-pr docs/superpowers/feat-enterprise-superpowers-customization/2026-06-25-enhance-superpowers-collection-pr-description.md
```

Expected:
- finish-spec-pr Step 1-2 で pr-description.md が Read される
- Step 3 で commit 差分が一覧表示される (本 plan の 1-12 タスクで作った commit 群)
- Step 4 で title が「feat: enhance-superpowers コレクションを新設」等で 1 問確認される
...
| spec セクション | 担当タスク |
|---|---|
| アーキテクチャ (enhance-superpowers/ 配下構造) | Task 2 (骨格) |
...
Plan complete and saved to `docs/superpowers/feat-enterprise-superpowers-customization/2026-06-25-enhance-superpowers-collection-plan.md`.
