...
- **ADR 番号の採番規約**（連番を保つことが目的）：
  ...
  - **例外：未 merge の ADR（main に入っていないもの）は改番してよい。** 番号が衝突したら改番するしかない（先に merge された PR が番号を確定させるため）。上記の禁止が守るのは SKILL.md・CONTEXT.md・他 ADR・公開済みリリースノートなど**repo 外にまで広がる参照**であり、未 merge の ADR は参照が branch 内に閉じているので該当しない。
