...
7. **初回 merge 後、release draft の tag を `v0.1.0` → `v0.0.1` に付け替える**（新規コレクションは直前のリリースが無いため version-resolver の patch 固定が効かず、release-drafter の初期値 `0.1.0` が出る）：`gh release edit <collection>/v0.1.0 --tag <collection>/v0.0.1 --title <collection>/v0.0.1`
