...
> **ADR-0022 で再構成**：本表は非職種（プロダクトクリティーク）・混成（リスク・法務・品質）・冗長（デザインシステムエンジニア／機能別詳細素材）を含む。最終 S1 roster は ADR-0022（実在職種5体＋ディレクター）を参照。
...
- **デザイン層の出口は moodboard ではなく design-system ＋ design-tokens（機械可読）**。Claude Design にトークンを渡し、見た目を自前発明させずトークンに準拠させる。（**ADR-0020 で改訂**：出口は repo-root の具体デザイン憲法 `DESIGN.md` に集約。moodboard 廃止の射程は「Canvas 機構の廃止／美的画像入力の必要は存続」と明確化）
- **画面一覧はプロトタイプの枠組み（骨格）**。エージェントがドラフト → **人間が枠組みレビュー（G4 相当の軽い関所）** → 以降自律（screen-specs／ブリーフ）。骨格ミスの是正を最も安い段階で行う（ADR-0007 の「枠組みは人間」に整合）。
- **premise シフト**：対話→自律 self-grill ／ Claude Chat→Claude Code ／ Obsidian→repo-native（ADR-0010・0012）／ prototype-builder→Claude Design（ADR-0003）／ 人間＝**アンカーを対話で決める（書くのは AI・ADR-0012）**＋ゲート。維持：番号固定ページ・真実源を 1 つ・カバレッジ逆引き。
- **出力レイアウト**：repo の `docs/discovery/` に集約。`anchors/`（G1 対話から AI が書く）・`planning/`（企画層・番号固定）・`design/`（`screens.md`・`screen-specs/<area>/`）・`brief.md`。（`DECISIONS.md` は ADR-0019 で廃止／`feature-details.md` は ADR-0021 で廃止／視覚デザイン design-concept/system/tokens は ADR-0020 で repo-root `DESIGN.md` へ集約）
- **完全性ガード**：ディレクターが期待マニフェストを持ち、各成果物を ✅生成合格／➖省略(理由)／⚠️未達(理由) の3状態で決着させる。評価（プロダクトクリティーク）は最大3ラウンド差し戻し（findings 付きで同一職種を continuation 再起動・該当ファイル改訂）→未達は decide-record-proceed。依存順厳守（上流が評価合格してから下流着手）。ゲート（画面一覧レビュー・G2）で人間にギャップレポート（➖省略/⚠️未達 の一覧）を必ず提示し、黙って欠落が通過しないようにする。
