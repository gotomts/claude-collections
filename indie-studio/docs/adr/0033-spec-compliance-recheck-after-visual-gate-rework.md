...
- **scope は凍結**。ただし **2 層**で見る：
  1. **変更されたセクション / token の spec compliance**
  2. **変更 diff が波及しうる document-level の形式不変条件** — セクション順序（正規 8 セクションの並び）／重複見出しの有無／frontmatter のフラット map（ネスト禁止）／Dimension の unit suffix／variant の hyphen 連結／英語単独セクション名
  - ステップ 2 で決着済みの**内容**（意味・妥当性）は再評価しない。**形式**は文書全体で見る。token 1 個の書き換えが frontmatter の map 構造や見出し重複を壊しうるため、セクション単位に閉じては検出できない。
- **形式違反があれば** `indie-studio:product-designer` を continuation 再起動して修正 → 再チェック。このループは**視覚ゲートの 2 ループ制限とは別枠**とする（形式適合には妥協点がないため、人間の往復とは別に収束させる）。
- **失敗上限は「再チェック 3 回」**（finding 単位ではなく**ループ全体の回数**）。finding が入れ替わりながら発生しても 3 回で打ち切る。
- **3 回で合格しなければ ⚠️未達として終端する**：
  - **mock を再生成しない**（stale mock で視覚ゲートに戻さない）
  - **再ゲートに進まない・S2 へ進めない**
  - ディレクターが未解消 findings を終端レポートに提示し、**人間の判断を仰いで停止する**（黙って通さない）
