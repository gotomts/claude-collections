...
accepted（ADR-0016 の DESIGN.md 定義/タイミング/作成者を supersede。ADR-0011 のデザイン層出口を改訂。**決定 5 は ADR-0023 で resolved**）
...
5. **resolved＝DESIGN.md の生成機構（ADR-0023 で確定）**。生成機構は新スキル `design-direction`（S1 と S2 の間・S1.5）として実装する。担い手：ディレクター（`design-direction` スキル本体）＋ `product-designer`（拡張：direction-pick 対話と DESIGN.md compose）＋ `visual-designer`（新規・実在職種：参考画像から mood / palette / typography vibe 構造化抽出）＋ `reviewer`（既存）。画像入力経路は Claude Code チャットの Vision API 添付または絶対 path 指定。**ADR-0011 の「moodboard 廃止」の射程は維持**（Obsidian Canvas 機構の廃止）し、画像入力による mood の人間ハンドオフは `design-direction` 内の専用ステップ（direction-pick 3 問 ＋ 画像 ingest）で吸収する。詳細：ADR-0023。
