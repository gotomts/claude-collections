...
accepted（ADR-0011 / ADR-0016 の planning・design レイアウトを確定。ADR-0020 と併走。**ADR-0028 で `docs/discovery/` は `docs/indie-studio/discovery/` 配下に namespace 化**：本 ADR の内部構造（`planning/` ・ `design/` ・ `screen-specs/<area>/` 等）は namespace prefix の下にそのまま保持）
