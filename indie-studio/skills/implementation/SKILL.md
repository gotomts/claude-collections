...
- **共通**:
  ...
  - **進行 protocol**: **停止ゼロ**（実装途中で人間に聞かない・ADR-0004）／曖昧点は decide-record-proceed＝根拠ある仮定を置き**仮定を PR に明記**／設計の穴・screen-specs 曖昧は仮定明記、ADR 候補は ADR を書いて PR で晒す。
  - **入力の所在**: チケット（Linear・依存順・受入条件・冪等キー `S-{nn}`）／技術設計 docs（`docs/indie-studio/tech/`・`docs/adr/`・`CONTEXT.md`・screen-specs・`DESIGN.md`）。
- **開発職種**: **S3 のスタックに応じて該当分のみ並列起動**。垂直スライス＝画面〜ドメイン〜データ〜test を貫く（unit/widget/受入/E2E を自分で書く）。**テスト戦略は S3 が決めた値を渡す**（Web=Playwright／モバイル=Maestro・主要フローに絞る）。**push/PR はしない**（実装＋テスト＋ローカル commit まで。push＋PR open はディレクターのみ・権限分離）。
- **評価職種（3観点）**: **評価観点**＝code-reviewer=品質／security-engineer=セキュリティ／performance-engineer=性能（**性能影響時のみ起動**）。**受入条件（チケット）充足**を ✅/➖/⚠️ で。**差し戻し protocol**＝round1 fresh→凍結 continuation・最大 3R（ADR-0018）。
