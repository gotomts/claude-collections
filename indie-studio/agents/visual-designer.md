...
- `design-principles` から「優先する価値」（例：精度 > 親しみ）と「禁じ手」を読み、温度 / 密度 / 形式性の方向を**1 つ仮決**＋ specific reference 2 個併記で返す。
- WCAG / CVD は適用不可なので「画像なし＝事後検証必要」と明記。
- product-designer 側で direction-pick 3 問の **推奨** に流す（visual-designer が決め切るわけではない）。

## self-grill 観点

- **specific reference が固有名で出ているか**（人物・作品・年代・カタログ・建築・運動）。「modern / clean / trustworthy」は禁止語（AI defaults・形容詞列挙の罠）。
...
5. specific reference 1-3 個（固有名）。
