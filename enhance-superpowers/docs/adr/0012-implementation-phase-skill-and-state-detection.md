...
新 skill **`enhance-executing-plans`** を追加する (superpowers 公式の brainstorming → writing-plans → **executing-plans** の 3 段目と対をなす命名)。

責務:

- **実装前**: `software-architect` を能動 dispatch (実装方針の Clean Architecture / SOLID 整合性の pre-flight review)
- **実装本体**: `superpowers:executing-plans` を invoke (Y 方式 = superpowers に委譲、ADR-0006 と同型)
- **実装完了ごと (slice 単位)**: `code-reviewer` を能動 dispatch (実装コードの review)
- **セキュリティ箇所**: `security-engineer` を常時能動 dispatch (実装対象 slice に auth / crypto / データ取扱 / 外部入力等の変更が含まれる場合)
- **dispatch log**: plan.md 末尾「## レビュー履歴」セクションに追記 (ADR-0007 の pattern に従う、実装フェーズ dispatch は plan.md に集約)
- **完了後**: `gwt-test` skill を chain invoke (skill chain 継続)

STOP POINT 1 の性質を変更: 「人間 / 実装 AI に完全委譲」→ 「新 skill が実装前後の agent 能動 dispatch を強制し、実装本体を superpowers:executing-plans に委譲」。
