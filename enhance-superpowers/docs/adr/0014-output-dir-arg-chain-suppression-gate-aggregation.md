...
`enhance-brainstorming` / `enhance-executing-plans` に 3 つの拡張を入れる — (E1) 成果物の出力先引数化、(E2) 実装フェーズへの chain 抑止、(E3) 承認 gate の集約 mode。**E1 と E3 は引数省略時に従来挙動を完全に維持する。E2 のみ既定挙動を意図的に変更し**、引数省略時も Step 7 で「実装へ進むか / Spec で止めるか」の 1 問確認を追加する（chain を止める手段が存在しなかった状態の是正）。
...
**3 つの引数を追加する。E1 / E3 は省略時に現行挙動と完全に同一。E2 のみ、省略時にも 1 問確認が 1 回増える**（下記 E2 参照。yes を選べば従来と同じ流れになるため、既存の呼び出しが壊れることはない）。
